...
export function StoreProvider({ children }) {
    const store = useLocalStore(() => ({
        ...
        //Temporal
        /*temporalLogin: () => {
            temporal.login(temporal_username, temporal_password)
                .then(() => {
        ...
        uploadFile: (fileInput) => {
            ...
            fetch("https://dev.api.temporal.cloud:6771/api/v2/ipfs/public/file/add", requestOptions)
            .then(response => response.text())
            .then(result => {
                store.uploadAddress = result;
                store.uploadState = "done";
                console.log(result)
            })
            .catch(error => {
